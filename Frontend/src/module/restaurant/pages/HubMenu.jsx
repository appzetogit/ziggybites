...
export default function HubMenu() {
        ...
        try {
          ...
          const normalizedSections = menuData.map((section, index) => ({
            ...
            items: Array.isArray(section.items) ? section.items.map(item => ({
              ...
              macronutrients: item.macronutrients && typeof item.macronutrients === 'object' ? {
                protein: item.macronutrients.protein ?? null,
                carbohydrate: item.macronutrients.carbohydrate ?? null,
                fat: item.macronutrients.fat ?? null,
                fibre: item.macronutrients.fibre ?? null,
              } : null,
              vitamins: item.vitamins && typeof item.vitamins === 'object' ? {
                vitaminA: item.vitamins.vitaminA ?? null,
                vitaminB1: item.vitamins.vitaminB1 ?? null,
                vitaminB2: item.vitamins.vitaminB2 ?? null,
                vitaminB3: item.vitamins.vitaminB3 ?? null,
                vitaminB5: item.vitamins.vitaminB5 ?? null,
                vitaminB6: item.vitamins.vitaminB6 ?? null,
                vitaminB7: item.vitamins.vitaminB7 ?? null,
                vitaminB9: item.vitamins.vitaminB9 ?? null,
                vitaminB12: item.vitamins.vitaminB12 ?? null,
                vitaminC: item.vitamins.vitaminC ?? null,
                vitaminD: item.vitamins.vitaminD ?? null,
                vitaminE: item.vitamins.vitaminE ?? null,
                vitaminK: item.vitamins.vitaminK ?? null,
              } : null,
  ...
  const handleSaveCategoryName = () => {
    ...
    // Update menuData so category name reflects in UI
    setMenuData(prev => prev.map(g => g.id === selectedCategory.id ? { ...g, name: newCategoryName } : g))
  ...
  const closeAllCategories = () => setExpandedGroups(new Set())
  const expandAllCategories = () => setExpandedGroups(new Set(menuData.map(g => g.id)))
        ...
        {activeTab === "add-ons" ? (
          ...
          <div className="px-4 space-y-4">
            {filteredMenuGroups.length > 0 && (
              <div className="flex items-center justify-end gap-2 py-1">
                <button
                  ...
                  className="text-xs font-medium text-gray-600 hover:text-gray-900 px-2 py-1 rounded hover:bg-gray-100"
                >
                  Close all
                </button>
                <button
            ...
            {filteredMenuGroups.map((group) => {
              ...
              const varietyCount = group.items.reduce((sum, item) => sum + (item.variations?.length || 0), 0)
                  ...
                  <div className="py-3 flex items-center justify-between px-4">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <h3 className="text-base font-bold text-gray-900">
                        {group.name} (Items: {enabledItems} | Varieties: {varietyCount})
                      </h3>
                    </div>
            ...
            >
              ...
              <div className="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4 flex items-center gap-3">
                ...
                <button
                  ...
                  disabled={!addonName.trim() || !addonDescription.trim() || !addonPrice || addonImages.length === 0 || uploadingAddonImages}
