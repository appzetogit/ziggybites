 ...
 * @param {boolean} stickyFooter - Content uses flex layout so child can pin bottom (e.g. Okay button); no scroll on whole sheet (default: false)
 */
export default function BottomPopup({
  ...
  stickyFooter = false // When true, content area is flex so child can pin footer (bottom not scrollable)
}) {
      ...
      {isOpen && (
        <>
          ...
          >
            ...
            {/* Content – stickyFooter: flex col so child can pin bottom; else scrollable */}
            {!isCollapsed ? (
              <div className={`flex-1 min-h-0 px-4 py-4 ${stickyFooter ? "flex flex-col overflow-hidden" : "overflow-y-auto"}`}>
                {children}
              </div>
            ) : (
