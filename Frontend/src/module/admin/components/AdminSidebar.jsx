...
const adminBackendUrl = API_BASE_URL?.replace("/api", "") || "http://localhost:5000";
...
export default function AdminSidebar({ isOpen = false, onClose, onCollapseChange }) {
  ...
  const [unassignedBadge, setUnassignedBadge] = useState(0)
  const adminSocketRef = useRef(null)

  useEffect(() => {
    const socket = socketIO(adminBackendUrl, {
    ...
    socket.on("unassigned_order", () => {
      setUnassignedBadge((prev) => prev + 1)
    })
  ...
  useEffect(() => {
    if (location.pathname === "/admin/unassigned-orders") {
      setUnassignedBadge(0)
    }
  }, [location.pathname])
    ...
    if (item.type === "link") {
      ...
      return (
        ...
        >
          ...
          {!isCollapsed && item.path === "/admin/unassigned-orders" && unassignedBadge > 0 && (
            <span className="ml-auto inline-flex items-center justify-center w-5 h-5 text-[10px] font-bold text-white bg-red-500 rounded-full">
              {unassignedBadge > 9 ? "9+" : unassignedBadge}
            </span>
          )}
        </Link>
      )
