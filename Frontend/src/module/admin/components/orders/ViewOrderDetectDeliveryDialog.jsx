...
import { X, Clock, CheckCircle, XCircle, User, Phone, Package, MapPin, Loader2, Check } from "lucide-react"
...
export default function ViewOrderDetectDeliveryDialog({ isOpen, onOpenChange, order, onOrderUpdated }) {
  const [isMarkingReady, setIsMarkingReady] = useState(false)
  if (!isOpen || !order) return null

  const StatusIcon = getStatusIcon(order.status)
  const original = order.originalOrder
  const canMarkReady = original && ['preparing', 'confirmed'].includes(original.status) && !original.tracking?.ready?.status

  const handleMarkReady = async () => {
    if (!original) return
    const id = original._id || original.orderId
    if (!id) return
    setIsMarkingReady(true)
    try {
      await adminAPI.markOrderReady(id)
      toast.success("Order marked as ready. It will now show on the delivery partner side.")
      onOrderUpdated?.()
      onOpenChange(false)
    } catch (err) {
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-slate-200 bg-slate-50">
          {canMarkReady && (
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-60 transition-all"
            >
              {isMarkingReady ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
              Mark as ready
            </button>
          )}
          <button
