...
import { Eye, Printer, ArrowUpDown, Phone, User, Check, Loader2 } from "lucide-react"
...
export default function OrderDetectDeliveryTable({ orders, visibleColumns, onViewOrder, onPrintOrder, onMarkReady }) {
                ...
                {visibleColumns.actions && (
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="flex items-center justify-center gap-2 flex-wrap">
                      {order.originalOrder && ['preparing', 'confirmed'].includes(order.originalOrder.status) && !order.originalOrder.tracking?.ready?.status && onMarkReady && (
                        <button
                          onClick={async () => {
                            const id = order.originalOrder._id || order.originalOrder.orderId
                            if (!id) return
                            setMarkingOrderId(id)
                            try {
                          ...
                          disabled={markingOrderId !== null}
                          className="inline-flex items-center gap-1 px-2 py-1.5 rounded text-xs font-medium bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-60 transition-colors"
                          title="Mark as ready (shows on delivery partner side)"
                        >
                          {markingOrderId === (order.originalOrder._id || order.originalOrder.orderId) ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
                          Mark as ready
                        </button>
                      )}
                      <button 
